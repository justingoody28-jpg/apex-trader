...
function LosersTab(props){
  ...
  var [validation,setValidation]=useState({});  // ticker -> {score,checks,loading}
  var [validating,setValidating]=useState(false);
  var [summary,setSummary]=useState(null);

  function vscore(v){
    if(v==="Strong Overreaction")return 5;
    if(v==="Overreaction")return 4;
    if(v==="Partial Overreaction")return 3;
    if(v==="Mixed")return 2;
    return 1;
  }
  ...
  function validateLosers(losersArr){
    var fhk=props.fhKey||"";
    var fmpk=props.fmpKey||"";
    if(!fhk&&!fmpk) return; // no keys, skip validation
    setValidating(true);
    var results={};
    var pending=losersArr.length;
    function done(ticker,data){
      results[ticker]=data;
      pending--;
      if(pending<=0){setValidation(results);setValidating(false);}
    }
    losersArr.forEach(function(l){
      ...
      // Finnhub: analyst target + recommendation + insider sentiment
      if(fhk){
        promises.push(
          fetch("/api/market?source=fh&endpoint=stock/price-target?symbol="+t)
          .then(function(r){return r.json();})
          .then(function(d){
            if(d.targetMean&&l.dropNum){
              var curPrice=parseFloat(l.price.replace("$",""))||0;
              var upside=curPrice>0?((d.targetMean-curPrice)/curPrice*100):0;
              checks.push({label:"Analyst Target",value:"$"+d.targetMean.toFixed(0)+" ("+( upside>=0?"+":"")+upside.toFixed(1)+"%)",pass:upside>15,icon:upside>15?"✅":"❌"});
            }
          }).catch(function(){})
        );
        promises.push(
          fetch("/api/market?source=fh&endpoint=recommendation?symbol="+t)
          .then(function(r){return r.json();})
          .then(function(d){
            if(d&&d[0]){
              var rec=d[0];
              var buys=(rec.strongBuy||0)+(rec.buy||0);
              var sells=(rec.strongSell||0)+(rec.sell||0);
              var total=buys+sells+(rec.hold||0);
              var pct=total>0?Math.round(buys/total*100):0;
              checks.push({label:"Analyst Consensus",value:pct+"% Buy ("+buys+" buys)",pass:pct>=50,icon:pct>=50?"✅":"❌"});
            }
          }).catch(function(){})
        );
        promises.push(
          fetch("/api/market?source=fh&endpoint=stock/insider-sentiment?symbol="+t+"&from=2024-01-01")
          .then(function(r){return r.json();})
          .then(function(d){
            if(d&&d.data&&d.data.length>0){
              var recent=d.data[d.data.length-1];
              var mspr=recent.mspr||0;
              checks.push({label:"Insider Sentiment",value:"MSPR "+mspr.toFixed(1)+(mspr>0?" (buying)":" (selling)"),pass:mspr>0,icon:mspr>0?"✅":"❌"});
            }
          }).catch(function(){})
        );
        promises.push(
          fetch("/api/market?source=fh&endpoint=stock/earnings?symbol="+t)
          .then(function(r){return r.json();})
          .then(function(d){
            if(d&&d[0]){
              var beat=d[0].actual>d[0].estimate;
              var surprise=d[0].estimate>0?((d[0].actual-d[0].estimate)/Math.abs(d[0].estimate)*100):0;
              checks.push({label:"Last Earnings",value:(beat?"Beat":"Miss")+" by "+Math.abs(surprise).toFixed(1)+"%",pass:beat,icon:beat?"✅":"❌"});
            }
          }).catch(function(){})
        );
      }

      // FMP: revenue growth + P/E + free cash flow
      if(fmpk){
        promises.push(
          fetch("/api/market?source=fmp&endpoint=income-statement/"+t+"?limit=2")
          .then(function(r){return r.json();})
          .then(function(d){
            if(d&&d.length>=2){
              var growth=d[1].revenue>0?((d[0].revenue-d[1].revenue)/d[1].revenue*100):0;
              checks.push({label:"Revenue Growth",value:(growth>=0?"+":"")+growth.toFixed(1)+"% YoY",pass:growth>0,icon:growth>0?"✅":"❌"});
            }
          }).catch(function(){})
        );
        promises.push(
          fetch("/api/market?source=fmp&endpoint=ratios-ttm/"+t+"?")
          .then(function(r){return r.json();})
          .then(function(d){
            if(d&&d[0]){
              var fcf=d[0].freeCashFlowPerShareTTM;
              var pe=d[0].peRatioTTM;
              if(fcf!==undefined) checks.push({label:"Free Cash Flow",value:fcf>0?"Positive ($"+fcf.toFixed(2)+"/sh)":"Negative ($"+fcf.toFixed(2)+"/sh)",pass:fcf>0,icon:fcf>0?"✅":"❌"});
              if(pe!==undefined&&pe>0) checks.push({label:"P/E Ratio",value:pe.toFixed(1)+"x",pass:pe<30,icon:pe<30?"✅":"❌"});
            }
          }).catch(function(){})
        );
      }
      ...
      Promise.all(promises).then(function(){
        ...
        var confidence=score>=75?"HIGH":score>=50?"MEDIUM":"LOW";
        done(t,{checks:checks,score:score,confidence:confidence,pass:pass,total:total});
      });
    });
  }

  function runAnalysis(){
    setLoading(true);setError(null);setLosers([]);setSummary(null);
    var userPrompt="Today is "+new Date().toDateString()+". Identify the 8 biggest US stock losers over the past 5 trading days and provide deep fundamental analysis for each.\n\n"+
    ...
    .then(function(data){
      ...
      // Prepend the primed "[{" and find full array
      var combined="[{"+raw;
    ...
    .catch(function(err){var m=err.message||"Unknown error";setLoading(false);setError("Analysis failed: "+m+(m.includes("401")?" — Invalid API key.":" — Try clicking Run AI Analysis again."));});
  }

  var shown=losers.filter(function(l){
    if(filter==="over")return l.verdict==="Strong Overreaction"||l.verdict==="Overreaction";
    if(filter==="watch")return l.verdict==="Partial Overreaction"||l.verdict==="Mixed";
    if(filter==="just")return l.verdict==="Justified";
    return true;
  }).sort(function(a,b){return vscore(b.verdict)-vscore(a.verdict);});

  return(
    <div style={{animation:"fu 0.3s ease"}}>
      <div style={{marginBottom:20,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:14,alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:20,fontWeight:700,color:"#f1f5f9"}}>AI Losers Analysis</div>
          <div style={{fontSize:11,color:"#334155",marginTop:3}}>Claude identifies the week's biggest losers and analyzes each drop</div>
        </div>
        <button onClick={runAnalysis} disabled={loading} style={{background:loading?"#1e293b":"linear-gradient(135deg,#1d4ed8,#7c3aed)",border:"none",color:loading?"#475569":"#fff",borderRadius:9,padding:"11px 22px",fontSize:13,fontWeight:700,cursor:loading?"not-allowed":"pointer"}}>
          {loading?"Analyzing...":losers.length>0?"Refresh Analysis":"Run AI Analysis"}
        </button>
      </div>
      {!loading&&losers.length===0&&!error&&(
        <div style={{background:"#0a0f1a",border:"1px dashed #1e293b",borderRadius:16,padding:"52px 32px",textAlign:"center"}}>
          <div style={{fontSize:16,fontWeight:700,color:"#f1f5f9",marginBottom:10}}>Market Losers Intelligence</div>
          <div style={{fontSize:13,color:"#475569",maxWidth:460,margin:"0 auto",lineHeight:1.7,marginBottom:28}}>Claude scans this week's biggest losers, analyzes the fundamental cause of each drop, and verdicts whether it is a buying opportunity or a falling knife.</div>
          <button onClick={runAnalysis} style={{background:"linear-gradient(135deg,#1d4ed8,#7c3aed)",border:"none",color:"#fff",borderRadius:9,padding:"13px 32px",fontSize:14,fontWeight:700,cursor:"pointer"}}>Run AI Analysis</button>
        </div>
      )}
      {loading&&<div style={{background:"#0a0f1a",border:"1px solid #0f172a",borderRadius:12,padding:"24px 22px",display:"flex",alignItems:"center",gap:14}}><div style={{width:16,height:16,border:"2px solid #1e293b",borderTopColor:"#3b82f6",borderRadius:"50%",animation:"spin 0.8s linear infinite",flexShrink:0}}/><div><div style={{fontSize:13,color:"#f1f5f9",fontWeight:600}}>Claude is analyzing this week's biggest losers...</div><div style={{fontSize:11,color:"#334155",marginTop:3}}>Scanning earnings, analyst targets, and fundamentals</div></div></div>}
      {error&&<div style={{background:"#1c0505",border:"1px solid #b91c1c",borderRadius:12,padding:"16px 22px",marginBottom:16}}><div style={{fontSize:13,fontWeight:700,color:"#f87171",marginBottom:4}}>Error</div><div style={{fontSize:12,color:"#fca5a5"}}>{error}</div></div>}
      {summary&&!loading&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:18}}>
          <MC label="Stocks Analyzed" value={summary.total}/>
          <MC label="Overreactions" value={summary.over+"/"+summary.total} color="#4ade80"/>
          <MC label="Avg Weekly Drop" value={summary.avgDrop+"%"} color="#f87171"/>
          <MC label="Avg Upside" value={"+"+summary.avgUp+"%"} color="#4ade80"/>
        </div>
      )}
      {losers.length>0&&!loading&&(
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          {[{k:"all",l:"All"},{k:"over",l:"Overreactions"},{k:"watch",l:"Watch"},{k:"just",l:"Justified"}].map(function(f){
            return<button key={f.k} onClick={function(){setFilter(f.k);}} style={{background:filter===f.k?"#1e293b":"transparent",border:"1px solid "+(filter===f.k?"#334155":"#0f172a"),color:filter===f.k?"#f1f5f9":"#475569",borderRadius:6,padding:"6px 13px",fontSize:11}}>{f.l}</button>;
          })}
        </div>
      )}
          ...
          <div key={l.ticker+i} style={{background:"#0a0f1a",border:"1px solid #0f172a",borderRadius:14,padding:"20px 22px",marginBottom:12,animation:"fu 0.3s ease "+(i*0.05)+"s both"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:10}}>
              <div style={{display:"flex",gap:14,alignItems:"center"}}>
                ...
                <div><div style={{fontSize:22,fontWeight:800,color:"#ef4444"}}>{l.drop}</div><div style={{fontSize:10,color:"#475569",marginTop:2}}>5-day drop</div></div>
            ...
            {validation[l.ticker]&&(function(){
              ...
              return(
                <div style={{background:"#030712",border:"1px solid #0f172a",borderRadius:10,padding:"14px 16px",marginTop:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <div style={{fontSize:10,color:"#334155",letterSpacing:2}}>QUANTITATIVE VALIDATION</div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{fontSize:10,fontWeight:700,color:confColor,background:confBg,border:"1px solid "+confColor,borderRadius:4,padding:"2px 8px"}}>{conf} CONFIDENCE</div>
                      <div style={{fontSize:11,color:"#64748b"}}>{v.pass}/{v.total} confirm</div>
                    </div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                    {v.checks.map(function(c,ci){
                      return(
                        <div key={ci} style={{background:"#0a0f1a",borderRadius:6,padding:"7px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div>
                            <div style={{fontSize:9,color:"#334155",marginBottom:2}}>{c.label}</div>
                            <div style={{fontSize:11,color:"#94a3b8"}}>{c.value}</div>
                          </div>
                  ...
                  <div style={{marginTop:10,background:confBg,borderRadius:6,padding:"8px 12px"}}>
                    <div style={{fontSize:11,fontWeight:700,color:confColor}}>
                      {conf==="HIGH"?"✅ Data confirms AI verdict":""}
                      {conf==="MEDIUM"?"⚠️ Data partially confirms — research further":""}
                      {conf==="LOW"?"❌ Data contradicts AI verdict — caution advised":""}
                    </div>
                  </div>
                </div>
              );
            })()}
            {validating&&!validation[l.ticker]&&(
              <div style={{fontSize:10,color:"#334155",textAlign:"center",padding:"8px 0",marginTop:8}}>⏳ Fetching validation data...</div>
            )}
          </div>
        );
      })}
      {losers.length>0&&!loading&&<div style={{marginTop:14,padding:"10px 14px",background:"#0a0f1a",border:"1px solid #0f172a",borderRadius:8,fontSize:10,color:"#334155"}}>AI analysis is for educational purposes only. Not financial advice.</div>}
